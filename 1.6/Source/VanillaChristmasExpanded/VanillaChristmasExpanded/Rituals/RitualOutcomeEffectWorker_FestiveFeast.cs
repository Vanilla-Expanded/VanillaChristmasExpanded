...
{
    public class RitualOutcomeEffectWorker_FestiveFeast : RitualOutcomeEffectWorker_FromQuality
    {

        public RitualOutcomeEffectWorker_FestiveFeast()
        {
        }

        public RitualOutcomeEffectWorker_FestiveFeast(RitualOutcomeEffectDef def) : base(def)
        {
        }
        ...
        public override void Apply(float progress, Dictionary<Pawn, int> totalPresence, LordJob_Ritual jobRitual)
        {
            ...
            {
                FestiveFavorManager.Instance.AddFestiveFavor(totalPresence.Keys.Count * 25);
            }

            if (outcome.positivityIndex == 2)
            {
                FestiveFavorManager.Instance.AddFestiveFavor(totalPresence.Keys.Count * 100);
